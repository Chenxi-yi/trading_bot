...
## 策略规则
1. 主触发（必须）：短周期价格通道在长周期价格通道之上（状态条件）  
2. 量能确认（必须）：近2个交易日内至少1天放量  
3. 辅助：收盘价连续两天站在通道上沿  
...
## A/B 档规则
- A档：满足主触发 + 量能 + 其余严格条件（与原严格版一致）
- B档：满足主触发 + 量能，其余按总分降序排序
...
- `reports/daily/YYYY-MM-DD_us_a.csv`
- `reports/daily/YYYY-MM-DD_us_b.csv`
- `reports/daily/YYYY-MM-DD_hk_a.csv`
- `reports/daily/YYYY-MM-DD_hk_b.csv`
